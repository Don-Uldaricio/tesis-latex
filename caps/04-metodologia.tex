...
\hline
\texttt{dt} & Simulación neuronal & Continuo & Paso de integración temporal de la simulación neuronal, en milisegundos. \\
\hline
\texttt{encoding\_duration} & Simulación neuronal & Continuo & Duración de la ventana de codificación de entrada, en milisegundos. \\
\hline
\texttt{extra\_eval} & Simulación neuronal & Continuo & Tiempo adicional de simulación tras la ventana de codificación (duración de evaluación $=$ \texttt{encoding\_duration} $+$ \texttt{extra\_eval}). \\
\hline
...
$(arch, disC, disM, proM, disSM, proSM, sLower, sGap, wScale, dt, encDur, extraEval) \leftarrow trial.\text{suggest}(\ldots)$ \tcp*{arquitectura, operadores SMOEA, esparsidad inicial y simulación}
$sUpper \leftarrow \min(sLower + sGap,\ 1)$\;
$evalDur \leftarrow encDur + extraEval$\;
\If{$arch = $ Poisson}{
...
Los resultados se organizaron en cuatro conjuntos experimentales, cada uno definido para responder una pregunta de comparación distinta: el conjunto de \textit{referencia}, que sustenta la comparación principal entre MOEA-CKF y S-NSGA-II sobre SNN; el conjunto \textit{con remuestreo}, que evalúa si estimar el fitness mediante remuestreo mejora o empeora el HV respecto de la evaluación estándar; el conjunto \textit{ANN}, que compara la plataforma SNN frente a la implementación ANN original de PlatEMO/MATLAB; y el conjunto de \textit{rendimiento temporal}, que mide el tiempo de cómputo de ambos algoritmos bajo condiciones controladas de hardware e hiperparámetros.

Los conjuntos de \textit{referencia} y \textit{con remuestreo} incluyeron 31 corridas por combinación, pareadas por semilla. Ambos registraron HV, frentes de Pareto y convergencia; se diferenciaron únicamente en que el segundo calculó el fitness mediante remuestreo, lo que permitió comparaciones pareadas.
...
El análisis se organizó en tres experimentos, cada uno con sus enfoques, los que se encuentran resumidos en la Tabla~\ref{tab:angulos-comparacion}. El primer experimento, el principal, compara MOEA-CKF y S-NSGA-II, ambos ejecutados en SNN, considerando tres enfoques: HV, frentes de Pareto y convergencia, todos sobre el conjunto experimental de referencia. El segundo experimento comparó cada algoritmo entre la plataforma SNN y la implementación ANN original (PlatEMO). Por último, los \textit{otros experimentos} evaluaron el efecto del remuestreo y el tiempo de cómputo, este último bajo hiperparámetros compartidos y condiciones de hardware equitativas entre algoritmos.
...
\multirow{3}{*}{SNN vs. SNN}        & Hipervolumen      & ¿Qué algoritmo tiene mejor frente de Pareto (SNN)?                         & \multirow{3}{*}{Conjunto de referencia}\\
...
\multirow{2}{*}{Otros exp.} & Remuestreo        & ¿El remuestreo mejora o empeora el HV?                                     & Conjunto experimental de referencia vs. Conjunto experimental con remuestreo \\
...
Este experimento principal compara MOEA-CKF y S-NSGA-II, ambos ejecutados en SNN (C++) sobre el conjunto experimental de referencia, mediante tres enfoques complementarios: HV, frentes de Pareto y convergencia.
...
Además de los dos experimentos anteriores, se evaluó a cada algoritmo bajo condiciones adicionales: el efecto del remuestreo sobre el HV, y el tiempo de cómputo bajo hiperparámetros compartidos y hardware equitativo.
